...
<p align="center">
  <strong>企業級智慧虛擬助理平台 — 以 AI 驅動，整合 Eiva 高效能安全後端引擎，為企業打造高效率、高智能的對話式服務體驗。</strong>
</p>
...
**本系統的主要目的為建立一個 nutshell（沙盒容器）來進行企業 Agent 的操作，提供一個安全、封閉的企業 Agent 執行環境。這套系統特別為 IoT（物聯網）、Edge Computation（邊緣運算），以及企業內部專用的 Agent 而設計。**

EIVA 的後端核心採用 **[Eiva 🦀🦞](./backend/)** —— 一套以 Rust 編寫的高效能、安全優先的 AI Agent 作業系統，提供毫秒級啟動、~15MB 極低記憶體佔用，以及業界領先的多層次安全防護機制。
...
| **安全優先** | Eiva 多層安全架構，防範 Prompt Injection、資料洩漏等 AI 安全威脅 |
...
EIVA 的核心動力來自 **[Eiva](./backend/)** —— 一套以 Rust 打造的 **AI Agent 作業系統**，如同「Linux for AI Agents」，提供穩定、安全的基礎設施。
...
#### ⚡ 極致效能
| 指標 | Eiva | Node.js Agent | Python Agent |
|------|-----------|---------------|--------------|
...
| 文件 | 說明 |
|------|------|
| [`backend/README.md`](./backend/README.md) | Eiva 完整使用說明 |
...
1. 啟動 Eiva 閘道：`eiva gateway start`
...
Eiva WebSocket 協議規格：[`backend/docs/CLIENT_SPEC.md`](./backend/docs/CLIENT_SPEC.md)
...
| `WS` | `/api/v1/gateway` | Eiva 閘道 WebSocket |
...
Eiva 系統的分支與核心架構深受 [RustyClaw](https://github.com/rexlunae/RustyClaw) 專案的啟發與影響。我們誠摯感謝原作者與貢獻者們，提供瞭如此穩定且安全的 AI Agent 作業系統基礎。
...
Eiva 後端引擎同樣採用 [MIT License](./backend/LICENSE) 授權。
...
- **Eiva 社群**：[Discord](https://discord.com/invite/clawd)
