---
name: testing-eiva-cli
description: Test Eiva CLI commands end-to-end. Use when verifying CLI changes, swarm commands, new subcommands, or gateway error handling.
---
...
The CLI binary is in the `eiva` package (crate path: `crates/eiva-cli`).

```bash
# Minimal build (TUI only, no desktop/GPU deps):
cargo build -p eiva --no-default-features --features tui
...
```bash
eiva swarm templates    # List available swarm templates
eiva swarm list         # List active swarms (empty by default)
eiva swarm create [TEMPLATE]  # Create swarm (default: swarm)
eiva swarm send <SWARM> <MESSAGE>  # Send message to swarm
eiva swarm status <SWARM>   # Show swarm status
eiva swarm stop <SWARM>     # Stop a swarm
```
...
5. **Desktop UI:** SwarmPanel compiles but requires Dioxus desktop runtime. Verify via `cargo check -p eiva-desktop` on CI.
