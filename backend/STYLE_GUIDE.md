...
(`eiva-claw-core`, `eiva-cli`, `eiva-tui`, `eiva-desktop`). Rules here align
...
| Crate | Rule |
|-------|------|
| `eiva-claw-core`, `eiva-tui` | Use `thiserror` for typed errors. No `anyhow` in public API. |
...
(`Result<String, ToolError>`, see `eiva-claw-core/src/tools/error.rs`):
...
code (`eiva-claw-core`, `eiva-tui`). User-facing CLI output in `eiva-cli` is the
exception.
...
```rust
//! `eiva-claw-core` — shared configuration, gateway protocol, secrets management,
...
All shell/subprocess execution goes through the sandbox layer (`crates/eiva-claw-core/src/sandbox.rs`).
