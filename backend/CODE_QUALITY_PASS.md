...
- **Fixed every clippy error this surfaced** across the workspace:
  ...
  - **eiva-claw-core** (13 errors, only visible under `--features full`):
...
- Deleted `crates/eiva-claw-core/src/provider_registry.rs` (975 lines).
- Removed `pub mod provider_registry;` from `lib.rs`.
- Removed the `genai = "0.5.3"` dependency from `eiva-claw-core/Cargo.toml`.
...
Two optional features of `eiva-claw-core` do not build, independent of lint work, so the
...
| `cargo clippy -p eiva-claw-core --features full --all-targets` | ✅ PASS |
| `cargo test -p tokenjuice` | ✅ PASS (8/8) |
| `cargo clippy --all-targets --all-features` | ❌ cannot build — `qr`+`browser` broken upstream (§3) |
| `eiva-claw-core` lib unit tests | ⚠️ 4 pre-existing env-dependent failures (below), not caused by this pass |

Pre-existing flaky tests in `crates/eiva-claw-core/src/tools/mod.rs`
