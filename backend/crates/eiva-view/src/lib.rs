//! `eiva-view` — shared component-oriented view data types.
//!
...
// Client crates (`eiva-tui`, `eiva-desktop`) import these through
// `eiva_view` so the dependency is compiled once for the entire workspace
// tree rooted at `eiva-view`.
pub use anyhow;
pub use chrono;
pub use clap;
pub use dirs;
