//! Integration tests for `eiva-view` component data types.
//!
...
fn format_duration_is_human() {
    use eiva_view::format_duration_ms;
...
fn set_tool_live_status_targets_running_call_and_clears_on_result() {
    use eiva_view::DisplayMessageData;
...
fn format_bytes_is_human() {
    assert_eq!(eiva_view::format_bytes(512 * 1024), "512 KB");
    assert_eq!(eiva_view::format_bytes(145 * 1024 * 1024), "145 MB");
    assert_eq!(
        eiva_view::format_bytes(2 * 1024 * 1024 * 1024 + 300 * 1024 * 1024),
        "2.3 GB"
    );
}
