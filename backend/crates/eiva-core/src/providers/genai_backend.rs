...
/// Encode an assistant turn (text + tool calls) into Eiva's canonical,
...
/// Map a Eiva provider id onto a genai adapter. Anthropic, Google, and
...
/// decoding Eiva's canonical tool-call / tool-result encoding back into
...
/// Parse a canonical Eiva envelope, verifying the `__eiva_kind` tag.
fn parse_canonical(content: &str, kind: &str) -> Option<serde_json::Value> {
    ...
    if value.get("__eiva_kind").and_then(|v| v.as_str()) == Some(kind) {
        Some(value)
    } else {
        None
    }
...
/// Build genai tool definitions from Eiva's tool registry, reusing the
...
/// Convert a genai [`ToolCall`] into Eiva's [`ParsedToolCall`].
