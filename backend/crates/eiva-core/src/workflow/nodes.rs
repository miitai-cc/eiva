...
pub trait WorkflowNode: Send + Sync {
    async fn execute(&self, node: &Node, ctx: &mut WorkflowContext) -> anyhow::Result<NodeResult>;
}
...
impl WorkflowNode for StartNode {
    async fn execute(&self, _node: &Node, ctx: &mut WorkflowContext) -> anyhow::Result<NodeResult> {
...
impl WorkflowNode for EndNode {
    async fn execute(&self, _node: &Node, _ctx: &mut WorkflowContext) -> anyhow::Result<NodeResult> {
        Ok(NodeResult::End)
    }
}
...
impl WorkflowNode for AgentNode {
    async fn execute(&self, node: &Node, ctx: &mut WorkflowContext) -> anyhow::Result<NodeResult> {
        ...
        let system_msg = crate::gateway::ChatMessage::text("system", "You are a helpful AI assistant in an automated workflow.");
        let user_msg = crate::gateway::ChatMessage::text("user", &rendered_prompt);
        
        let (default_provider, default_model, base_url) = crate::runtime_ctx::get_model_info()
            .unwrap_or_else(|| ("openai".to_string(), model_name.to_string(), "https://api.openai.com".to_string()));

        let req = crate::gateway::ProviderRequest {
        ...
        let response = crate::providers::call_with_tools(&http_client, &req, None).await?;
...
impl WorkflowNode for McpNode {
    ...
    async fn execute(&self, node: &Node, ctx: &mut WorkflowContext) -> anyhow::Result<NodeResult> {
        ...
        if let Some(mcp_mgr) = crate::runtime_ctx::get_mcp_manager() {
