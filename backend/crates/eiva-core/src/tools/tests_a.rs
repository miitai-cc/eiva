...
fn ws() -> &'static Path {
    // In the workspace, CARGO_MANIFEST_DIR is crates/eiva-core.
...
fn test_write_file_and_read_back() {
    let dir = std::env::temp_dir().join("eiva_test_write");
...
fn test_edit_file_single_match() {
    let dir = std::env::temp_dir().join("eiva_test_edit");
    let _ = std::fs::remove_dir_all(&dir);
...
fn test_edit_file_no_match() {
    let dir = std::env::temp_dir().join("eiva_test_edit_no");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join("f.txt"), "aaa\nbbb\n").unwrap();
...
fn test_edit_file_multiple_matches() {
    let dir = std::env::temp_dir().join("eiva_test_edit_multi");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join("f.txt"), "aaa\naaa\n").unwrap();
...
fn test_list_directory() {
    let args = json!({ "path": "crates/eiva-core/src" });
...
fn test_search_files_finds_pattern() {
    let args = json!({ "pattern": "exec_read_file", "path": "crates/eiva-core/src", "include": "*.rs" });
...
fn test_search_files_no_match() {
    let dir = std::env::temp_dir().join("eiva_test_search_none");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn test_find_files_keyword_no_match() {
    let dir = std::env::temp_dir().join("eiva_test_find_kw");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
