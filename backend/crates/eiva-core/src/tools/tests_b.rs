...
fn test_audit_sensitive_runs() {
    let dir = std::env::temp_dir().join("eiva_test_audit");
...
fn test_secure_delete_nonexistent() {
    let args = json!({ "path": "/tmp/nonexistent_eiva_xyz" });
    let result = exec_secure_delete(&args, ws());
    assert!(result.is_err());
}
...
fn test_secure_delete_requires_confirm() {
    let dir = std::env::temp_dir().join("eiva_test_secdelete");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn test_secure_delete_with_confirm() {
    let dir = std::env::temp_dir().join("eiva_test_secdelete2");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
