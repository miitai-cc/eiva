...
pub fn run_onboard_wizard(
    ...
    println!();
    t::print_header("🦀  Eiva Onboarding  🦀");
    println!();
    ...
    println!();
    println!("  Give your Eiva agent a name. This appears in the TUI");
    println!("  title bar, authenticator app labels, and anywhere the");
    println!("  agent identifies itself.");
    println!();

    let current_name = if config.agent_name.is_empty() || config.agent_name == "Eiva" {
        None
    } else {
    ...
    if !name_input.is_empty() {
        ...
        config.agent_name = "Eiva".to_string();
    }
    println!(
        "  {}",
        t::icon_ok(&format!(
    ...
    if !vault_exists {
        ...
        println!();
        println!("  Eiva stores API keys, tokens, and other credentials");
        ...
        println!("     time Eiva starts, including when the gateway is");
                ...
                if let Some(key) = provided_key {
                    ...
                    if existing.is_some() {
                        ...
                        if key.trim().is_empty() {
                            ...
                            println!("      {}", t::accent_bright("eiva onboard"));
                        } else {
                            secrets.store_secret(secret_key, key.trim())?;
                            println!("  {}", t::icon_ok("API key stored securely."));
                        }
    ...
    // default template).  A previous `eiva tui` run may have already
