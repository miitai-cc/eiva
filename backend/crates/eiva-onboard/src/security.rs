...
pub(crate) fn setup_agent_ssh_key(
    ...
    if has_key {
        ...
        secrets.delete_credential("eiva_agent")?;
    }

    println!("{}", t::bold("Agent SSH key (optional):"));
    println!();
    println!("  Eiva can have its own Ed25519 SSH key, separate");
    ...
    if do_gen.trim().eq_ignore_ascii_case("y") {
        let comment = format!("eiva-agent@{}", whoami());
        let pubkey =
            secrets.generate_ssh_key("eiva_agent", &comment, AccessPolicy::WithApproval)?;
        println!();
