//! Command-line interface for the `eiva-gateway` binary: argument
...
#[command(
    name = "eiva-gateway",
    version,
    about = "Eiva gateway — run the WebSocket gateway in the foreground"
)]
...
pub(crate) struct RunArgs {
    ...
    /// Path to SSH host key (default: ~/.eiva/ssh_host_key)
    #[arg(long, value_name = "PATH")]
    pub(crate) ssh_host_key: Option<std::path::PathBuf>,
    /// Path to authorized_clients file (default: ~/.eiva/authorized_clients)
    #[arg(long, value_name = "PATH")]
        ...
        PairCommands::Qr { host } => {
            ...
            match generate_pairing_qr_ascii(&data) {
                Ok(qr) => {
                    ...
                    println!("{}", qr);
                    println!();
                    println!("Scan this QR code with a Eiva client to pair.");
