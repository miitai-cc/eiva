...
pub async fn upload_workspace_file(req: &mut Request, res: &mut Response) {
    ...
    if let Some(file) = req.file("file").await {
        ...
        res.render(Json(
            serde_json::json!({"status": "ok", "filename": filename}),
        ));
    } else {
        res.status_code(StatusCode::BAD_REQUEST);
...
pub async fn get_workspace_tree(_req: &mut Request, res: &mut Response) {
    ...
    let tree = tokio::task::spawn_blocking(move || build_tree_sync(&base, ""))
        .await
        .unwrap_or_else(|_| serde_json::json!({ "error": "failed to build tree" }));
