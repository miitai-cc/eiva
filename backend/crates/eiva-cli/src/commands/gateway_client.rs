...
#[command(after_help = "\
EXAMPLES:
  eiva ask 'What is 2+2?'
  echo 'Summarize this' | eiva ask --stdin
  eiva ask --model anthropic/claude-haiku 'Quick question'
  eiva ask --no-tools 'Just chat, no actions'
")]
...
pub(crate) async fn handle_ask(config: &Config, args: AskArgs) -> Result<()> {
    ...
    if prompt.is_empty() {
        anyhow::bail!("No prompt provided. Use `eiva ask 'your prompt'` or `--stdin`.");
    }
    ...
    let (ws_stream, _) = tokio_tungstenite::connect_async(url.to_string())
        .await
        .context("Failed to connect to gateway. Is it running? Try `eiva gateway start`")?;
    let (mut writer, mut reader) = ws_stream.split();
