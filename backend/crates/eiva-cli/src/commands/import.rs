//! `eiva import` — migrate configuration and credentials from an
...
pub struct ImportArgs {
    ...
    /// Eiva settings directory (default: ~/.eiva)
...
pub(crate) fn run_import(args: &ImportArgs, config: &mut Config) -> Result<()> {
    ...
    if !source_dir.exists() {
        anyhow::bail!(
            "OpenClaw directory not found: {}\n\
             Specify the path with: eiva import /path/to/.openclaw",
            source_dir.display()
        );
    }
    ...
    let target_dir = args
        ...
        .as_ref()
        .map(PathBuf::from)
        .unwrap_or_else(|| home.join(".eiva"));

    println!();
    println!("{}", "━".repeat(60).dimmed());
    println!("{}", "  Eiva Import Wizard".cyan().bold());
    println!("{}", "━".repeat(60).dimmed());
    println!();
    println!(
        "  {} {}",
