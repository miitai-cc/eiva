//! `eiva refresh-token` — re-import OAuth/session tokens from an
//! existing OpenClaw installation into the Eiva vault.
...
/// Arguments for `eiva refresh-token`.
#[derive(Debug, Args)]
...
pub(crate) fn run_refresh_token(args: &RefreshTokenArgs, config: &mut Config) -> Result<()> {
    ...
    if args.restart {
        ...
        println!(
            "{}",
            "Restart the gateway to use the new token: eiva gateway restart".dimmed()
        );
    }
