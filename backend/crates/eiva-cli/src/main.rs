//! `eiva` — main CLI entry point for Eiva.
//!
//! Provides the `eiva` binary (interactive chat / one-shot commands) and
//! the `eiva-gateway` daemon binary.
...
#[command(
    name = "eiva",
    version,
    about = "Eiva — lightweight agentic assistant",
    long_about = "Eiva — a super-lightweight super-capable agentic tool with improved security.\n\n\
                  Run without a subcommand to launch the TUI. Use `eiva desktop` for the desktop client."
)]
struct Cli {
    #[command(flatten)]
    common: CommonArgs,
...
enum Commands {
    ...
    /// Import an existing OpenClaw installation into Eiva
...
struct SetupArgs {
    /// Agent workspace directory (default: ~/.eiva/workspace)
...
SECURITY NOTE:
  ...
  For scripted/automated setups, prefer environment variables:
    ...
    ANTHROPIC_API_KEY=sk-xxx eiva onboard
...
enum GatewayCommands {
    ...
    Start {
        /// Log level filter for the gateway (e.g., "debug", "eiva=debug,info")
    ...
    Reload,
    /// Run the gateway in the foreground (like `eiva-gateway`)
    Run(GatewayRunArgs),
}
...
/// spawned client binary (`eiva-tui` / `eiva-desktop`), which parse
    ...
    match cli.command.unwrap_or(Commands::Tui(TuiArgs::default())) {
        ...
        Commands::Doctor(_args) => {
            ...
            if all_ok {
                ...
                println!(
                    "  Run {} or {} to fix.",
                    t::accent_bright("`eiva setup`"),
                    t::accent_bright("`eiva onboard`"),
                );
            }
        }

        // ── TUI ─────────────────────────────────────────────────
        // The TUI is a standalone binary (`eiva-tui`); locate and run it
        ...
        Commands::Tui(args) => {
            ...
            launch_client("eiva-tui", &fwd)?;
        }

        // ── Desktop ─────────────────────────────────────────────
        // The desktop client is a standalone binary (`eiva-desktop`); run
        ...
        Commands::Desktop(args) => {
            ...
            launch_client("eiva-desktop", &fwd)?;
        }
