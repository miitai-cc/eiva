...
//! `eiva-view` stays framework-agnostic for the TUI.
...
fn push_message(transcript: &mut ChatTranscript, msg: &ChatMessage) {
    let (role, payload) = match msg.role {
        ...
        MessageRole::Thinking => {
            let summary = match msg.duration_ms {
                Some(ms) => format!("Thought for {}", eiva_view::format_duration_ms(ms)),
    ...
    for tc in &msg.tool_calls {
        ...
        let name = match tc.duration_ms {
            Some(ms) => format!("{} · {}", base, eiva_view::format_duration_ms(ms)),
            None => base,
        };
        transcript.push(
            ChatRole::Assistant,
        ...
        if tc.result.is_none() {
            ...
            if let Some(line) = eiva_view::ToolCallData::from(tc).live_status_line() {
...
/// (see `execute_user_prompt` in eiva-gateway); rendered as a muted
