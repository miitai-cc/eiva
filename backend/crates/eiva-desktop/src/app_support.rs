...
pub(crate) fn handle_gateway_event(event: GatewayEvent, mut state: Signal<AppState>) {
    match event {
        ...
        GatewayEvent::HostInfo {
            ...
            state.write().host_info = Some(eiva_view::HostInfoData {
                hostname,
                os,
                arch,
                cpu_brand,
                ...
                gpus: gpus
                    .into_iter()
                    .map(|g| eiva_view::GpuDisplayInfo {
                        name: g.name,
                        vendor: g.vendor,
        ...
        GatewayEvent::LoadStatus {
            ...
            state.write().load_status = Some(eiva_view::LoadStatusData {
                load_score,
                avg_load_score,
                cpu_percent,
                memory_percent,
        ...
        GatewayEvent::ServiceList { services } => {
            state.write().services_data = Some(eiva_view::ServiceListData {
                services: services.into_iter().map(Into::into).collect(),
            });
        }
        GatewayEvent::ServiceActionResult { service, .. } => {
            if let Some(svc) = service {
                let info = eiva_view::ServiceInfoData::from(svc);
        ...
        GatewayEvent::EngineListResult { engines } => {
            ...
            let panel = s
                .engines_data
                .get_or_insert_with(eiva_view::EnginesPanelData::default);
        ...
        GatewayEvent::EnginePullProgress {
            ...
            let panel = s
                .engines_data
                .get_or_insert_with(eiva_view::EnginesPanelData::default);
            panel.pull_progress = Some(eiva_view::PullProgressData {
                engine,
                model,
                percent,
                downloaded_bytes,
        ...
        GatewayEvent::CronListResult { jobs } => {
            ...
            let panel = s
                .cron_data
                .get_or_insert_with(eiva_view::CronPanelData::default);
        ...
        GatewayEvent::MemoryListResult { entries } => {
            ...
            let panel = s
                .memory_data
                .get_or_insert_with(eiva_view::MemoryPanelData::default);
        ...
        GatewayEvent::McpListResult { servers } => {
            ...
            let panel = s
                .mcp_data
                .get_or_insert_with(eiva_view::McpPanelData::default);
        ...
        GatewayEvent::ChannelStatusResult { channels } => {
            ...
            let panel = s
                .channels_data
                .get_or_insert_with(eiva_view::ChannelsPanelData::default);
        ...
        GatewayEvent::ToolConfigResult { tools } => {
            ...
            let panel = s
                .tools_data
                .get_or_insert_with(eiva_view::ToolConfigPanelData::default);
        ...
        GatewayEvent::UsageStatsResult {
            ...
            let panel = s
                .analytics_data
                .get_or_insert_with(eiva_view::AnalyticsPanelData::default);
        ...
        GatewayEvent::LogsResult {
            ...
            let panel = s
                .logs_data
                .get_or_insert_with(eiva_view::LogsPanelData::default);
            panel.source = eiva_view::LogSource::from_wire(&source);
            panel.lines = lines;
...
pub(crate) fn build_directory_options(base_path: &str) -> Vec<eiva_view::DirectoryOption> {
    use std::path::Path;

    let mut options: Vec<eiva_view::DirectoryOption> = Vec::new();
    let base = Path::new(base_path);

    options.push(eiva_view::DirectoryOption {
