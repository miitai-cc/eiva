//! `eiva-desktop` — desktop GUI client for Eiva (Dioxus).
//!
//! Standalone binary: connects to a `eiva-gateway` over SSH and renders
//! the conversation in a native window. Launched directly or spawned by the
//! `eiva` CLI's `desktop` subcommand.
...
fn run(gateway_url: Option<String>, no_dialog: bool, pick_connection: bool) {
    ...
    tracing::info!("Starting Eiva Desktop");
