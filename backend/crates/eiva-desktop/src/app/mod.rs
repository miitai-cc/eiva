...
/// Bulma provides the component framework; `styles.css` layers the Eiva
...
pub fn App() -> Element {
    ...
    let connection_prefs = use_signal(eiva_view::ConnectionDialogData::load);
    ...
    use_effect(move || {
        ...
        if let Some(path) = current_dir {
            ...
            if let Some(root) = s.working_directory.as_deref() {
                s.file_browser = eiva_view::FileBrowserData::load(root);
            }
        }
    });
    ...
    let on_file_browser_select = move |path: std::path::PathBuf| {
        ...
        let attachment = eiva_view::PromptAttachment::from_file_path(path_str.clone());
        let mut s = state.write();
        if !s
            .prompt_attachments
            .iter()
    ...
    rsx! {
        ...
        div {
            ...
            div { class: "rc-workspace",
                if state.read().left_sidebar_visible {
                    Sidebar {
                        ...
                        tree: eiva_view::SidebarTree::build(
                            &state.read().projects,
                            &state.read().threads,
            ...
            div { class: "main",
                ...
                // as a top banner.
                for banner in eiva_view::build_banners(
                ...
                Chat {
                    ...
                    bottom_bar: eiva_view::BottomBarData {
                        ...
                        directory_selector: eiva_view::DirectorySelectorState {
                    ...
                    on_select_directory: move |path: String| {
                                    ...
                                    match std::env::set_current_dir(&selected) {
                                        Ok(()) => {
                                            let options = build_directory_options(&selected);
                                            {
                                                ...
                                                s.available_directories = options;
                                                s.file_browser =
                                                    eiva_view::FileBrowserData::load(
                                                        &selected,
                                                    );
                        ...
                        match std::env::set_current_dir(&path) {
                            Ok(()) => {
                                ...
                                s.available_directories = options;
                                s.file_browser = eiva_view::FileBrowserData::load(&path);
                                s.directory_selector_expanded = false;
                                s.directory_selector_error = None;
                                s.push_notice(
                                    MessageRole::Info,
