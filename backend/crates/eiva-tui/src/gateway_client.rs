...
pub(crate) fn gateway_event_to_gw_event(event: GatewayEvent) -> Option<GwEvent> {
    ...
    let ev = match event {
        ...
        E::HostInfo {
            ...
            GwEvent::HostInfo(eiva_view::HostInfoData {
                hostname,
                os,
                arch,
                cpu_brand,
        ...
        E::LoadStatus {
            ...
            memory_percent,
            summary,
        } => GwEvent::LoadStatus(eiva_view::LoadStatusData {
            load_score,
            avg_load_score,
            cpu_percent,
            memory_percent,
        ...
        E::SecretsListResult { entries, .. } => {
            let secrets: Vec<eiva_view::SecretInfoData> =
        ...
        E::ServiceList { services } => GwEvent::ServiceList(eiva_view::ServiceListData {
        ...
        E::EngineListResult { engines } => GwEvent::EngineListResult {
            engines: engines
                .into_iter()
                .map(|e| eiva_view::LocalEngineData {
                    id: e.id,
        ...
        E::EngineModelListResult { engine, models } => GwEvent::EngineModelListResult {
            ...
            models: models
                .into_iter()
                .map(|m| eiva_view::LocalModelData {
                    engine: engine.clone(),
                    name: m.name,
