...
pub fn TuiRoot(props: &TuiRootProps, mut hooks: Hooks) -> impl Into<AnyElement<'static>> {
    ...
    let hatching_dialog: State<eiva_view::HatchingDialogData> =
        hooks.use_state(eiva_view::HatchingDialogData::default);

    // ── Pairing dialog state ────────────────────────────────────────
    let show_pairing = hooks.use_state(|| false);
    let pairing_step: State<eiva_view::PairingStep> =
        hooks.use_state(|| eiva_view::PairingStep::ShowKey);
    let pairing_field: State<eiva_view::PairingField> =
        hooks.use_state(|| eiva_view::PairingField::Host);
    ...
    let prompt_attachments: State<Vec<eiva_view::PromptAttachment>> =
        hooks.use_state(Vec::new);

    // ── Info dialog state (secrets / skills / tool permissions) ──────
    let show_secrets_dialog = hooks.use_state(|| false);
    let secrets_dialog_data: State<Vec<eiva_view::SecretInfoData>> = hooks.use_state(Vec::new);
    ...
    let skills_dialog_data: State<Vec<eiva_view::SkillInfoData>> = hooks.use_state(Vec::new);
    ...
    let tool_perms_dialog_data: State<Vec<eiva_view::ToolPermInfoData>> =
        hooks.use_state(Vec::new);
    ...
    let host_info: State<Option<eiva_view::HostInfoData>> = hooks.use_state(|| None);
    let load_status: State<Option<eiva_view::LoadStatusData>> = hooks.use_state(|| None);
    let show_system_info = hooks.use_state(|| false);
    let show_services_dialog = hooks.use_state(|| false);
    let services_data: State<Option<eiva_view::ServiceListData>> = hooks.use_state(|| None);
    let show_engines_dialog = hooks.use_state(|| false);
    let engines_data: State<Option<eiva_view::EnginesPanelData>> = hooks.use_state(|| None);
    let engines_cursor = hooks.use_state(|| 0usize);
    let show_cron_dialog = hooks.use_state(|| false);
    let cron_data: State<Option<eiva_view::CronPanelData>> = hooks.use_state(|| None);
    let show_memory_dialog = hooks.use_state(|| false);
    let memory_data: State<Option<eiva_view::MemoryPanelData>> = hooks.use_state(|| None);
    let show_mcp_dialog = hooks.use_state(|| false);
    let mcp_data: State<Option<eiva_view::McpPanelData>> = hooks.use_state(|| None);
    let show_channels_dialog = hooks.use_state(|| false);
    let channels_data: State<Option<eiva_view::ChannelsPanelData>> = hooks.use_state(|| None);
    let show_analytics_dialog = hooks.use_state(|| false);
    let analytics_data: State<Option<eiva_view::AnalyticsPanelData>> =
        hooks.use_state(|| None);
    let show_logs_dialog = hooks.use_state(|| false);
    let logs_data: State<Option<eiva_view::LogsPanelData>> = hooks.use_state(|| None);
    ...
    element! {
        Root(
            ...
            composer: eiva_view::ComposerData {
            ...
            surface: eiva_view::ChatSurfaceData {
                is_processing: false,
                is_thinking: false,
            ...
            vault_unlock: eiva_view::VaultUnlockData {
            ...
            credential_request: eiva_view::CredentialRequestData {
            ...
            provider_selector: eiva_view::ProviderSelectorData {
                providers: provider_selector_items
                    ...
                    .map(|((display_name, id), auth_hint)| eiva_view::ProviderOptionData {
                        id,
                        display_name,
                        auth_hint,
                    })
            ...
            api_key_dialog: eiva_view::ApiKeyDialogData {
            ...
            device_flow: eiva_view::DeviceFlowData {
            ...
            model_selector: eiva_view::ModelSelectorData {
            ...
            pairing: eiva_view::PairingDialogData {
