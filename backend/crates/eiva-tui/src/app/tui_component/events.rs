...
pub(super) fn apply_gw_event(
    ...
    match ev {
        ...
        GwEvent::ThreadsUpdate {
            ...
            // orphan placement live entirely in eiva-view (one definition
            // shared with the desktop).
            let items: Vec<eiva_view::SidebarItemData> =
                thread_list.iter().map(item_from_thread).collect();
            let tree = eiva_view::SidebarTree::from_items(
                &projects.read(),
                items,
        ...
        GwEvent::ProjectsUpdate {
            ...
            let tree = eiva_view::SidebarTree::from_items(&projects.read(), items, active_id);
            threads.set(tree.into_flat_items());
        }
        ...
        GwEvent::ThreadHistory {
            ...
            if !ok {
                ...
                let converted: Vec<DisplayMessage> = eiva_view::convert_history(&history);
                tracing::debug!(
                    thread_id,
        ...
        GwEvent::ModelCompletionsLoaded { provider, models } => {
            ...
            if let Some(partial) = current_input.strip_prefix('/') {
                if partial.starts_with("model") {
                    let filtered =
                        eiva_view::build_slash_completions(&provider, Some(&models), partial);
        ...
        GwEvent::PairingSuccess { gateway_name } => {
            ...
            pairing_step.set(eiva_view::PairingStep::Complete);
        ...
        GwEvent::PairingError(err) => {
            ...
            pairing_step.set(eiva_view::PairingStep::EnterGateway);
        ...
        GwEvent::EnginePullProgress {
            ...
            if finished {
                ...
                data.pull_progress = Some(eiva_view::PullProgressData {
                    engine,
                    model,
                    percent,
                    downloaded_bytes,
        ...
        GwEvent::ShowLogs { source } => {
            ...
            data.source = eiva_view::LogSource::from_wire(&source);
            data.status = Some("Loading…".into());
...
/// [`SidebarItemData`](eiva_view::SidebarItemData).
///
/// This is the client's transport→view boundary; grouping and display logic
/// then live entirely in eiva-view.
fn item_from_thread(t: &crate::action::ThreadInfo) -> eiva_view::SidebarItemData {
    eiva_view::SidebarItemData {
        id: t.id,
