...
pub(super) fn apply_key_event(
    ...
    match event {
        TerminalEvent::Key(KeyEvent {
            ...
            // ── Pairing dialog ──────────────────────────────
            if show_pairing.get() {
                use eiva_view::{PairingField, PairingStep};
            ...
            if hatching_dialog.read().visible {
                let key = match code {
                    KeyCode::Enter => Some(eiva_view::HatchingKey::Enter),
                    KeyCode::Tab => Some(eiva_view::HatchingKey::Tab),
                    KeyCode::Esc => Some(eiva_view::HatchingKey::Escape),
                    KeyCode::Backspace => Some(eiva_view::HatchingKey::Backspace),
                    KeyCode::Char(c) => {
                        ...
                        Some(eiva_view::HatchingKey::Char(c))
                    }
                    _ => None,
                };
                ...
                if let Some(key) = key {
                    ...
                    match hatching.handle_key(key) {
                        eiva_view::HatchingEvent::Completed(result) => {
                        ...
                        eiva_view::HatchingEvent::Skipped => {
                            let mut m = messages.read().clone();
                        ...
                        eiva_view::HatchingEvent::Updated
                        | eiva_view::HatchingEvent::Ignored => {}
                    }
